...
class DataSetReader(object):
    ...
    def get_reader(self,
        ...
        def img_reader_m(img, size, mean, std):
            im = img['image']
    ...
    def get_reader_m(self,
        ...
        def reader():
            if mode == 'train':
                ...
                img['image'] = image
                img['id'] = 0
                im, im_id, im_shape = img_reader_m(img, size, cfg.pixel_means,
                                                 cfg.pixel_stds)
                batch_out = [(im, im_id, im_shape)]
                yield batch_out
...
def infer_m(size=416, image=None):
    return dsr.get_reader_m('infer', size, image=image)
